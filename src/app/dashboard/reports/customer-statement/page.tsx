        ...
        <body>
          ...
          <div class="statement-header">
            ...
            <div class="customer-info">
              ...
              ${data.customer.phone ? `<div class="detail">${data.customer.phone}</div>` : ''}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div>
            <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900">Customer Statement</h1>
            <p className="text-sm sm:text-base text-gray-600">Transaction history and balance for a customer</p>
          </div>
        </div>
        <button
          onClick={exportToPDF}
      ...
      {isLoading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
              ...
              <div className="space-y-2">
                <div>
                  <p className="text-xs sm:text-sm font-medium text-gray-900">{data.customer.name}</p>
                  {data.customer.address && (
                    <p className="text-xs text-gray-600">{data.customer.address}</p>
                  )}
                  {data.customer.phone && (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sm:p-8 text-center">
          ...
          <p className="text-gray-500 text-sm sm:text-base">No statement data available for the selected customer and period</p>
        </div>
      ) : (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sm:p-8 text-center">
          <DocumentTextIcon className="w-12 h-12 text-gray-400 mx-auto mb-4" />
          <p className="text-gray-500 text-sm sm:text-base">Select a customer to generate a statement</p>
        </div>
      )}
    </div>
  );
